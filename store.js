...
if (!window.supabase) {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (el) el.innerHTML = '<div class="empty">Could not load the store. Please reload the page.</div>';
    });
...
async function loadPhones() {
    ...
    if (error) {
        ...
        productsEl.innerHTML = '<div class="empty">Could not load listings. Please try again later.</div>';
        return;
    }

    window.allPhones = data || [];
    renderProducts();
}
...
function renderProducts() {
    ...
    productsEl.innerHTML = list.length ? list.map(p => `
        <article class="product" onclick="showProduct('${p.id}')">
            ...
            <h3>${esc(p.name)}</h3>
            <div class="meta">
                ${esc(p.storage || '')}${p.color ? ' • ' + esc(p.color) : ''}<br>
                Condition: ${esc(p.condition || 'Not specified')}<br>
                Battery Health: ${esc(p.battery || 'Not specified')}
            </div>
            <div class="price">₱${Number(p.price || 0).toLocaleString('en-PH')}</div>
            <div class="view-details">View Details →</div>
        </article>
    `).join('') : '<div class="empty">No iPhones found.</div>';
}
    ...
    details.innerHTML = `
      <div class="details-card">
        ...
        <div class="details-image">
          ${phone.images?.[0] ? `<img src="${esc(phone.images[0])}" alt="${esc(phone.name)}">` : `<div class="placeholder"></div>`}
        </div>
        <div class="details-info">
          ...
          ${phone.description ? `<div class="description"><strong>Description</strong><p>${esc(phone.description)}</p></div>` : ''}
          <div class="contact-box">
            <h3>Interested in this iPhone?</h3>
            <p>Contact us directly to arrange a meet-up.</p>
            <div class="contact-item">📘 <strong>Facebook:</strong>
              <a href="https://www.facebook.com/armanstephent" target="_blank" rel="noopener" onclick="event.stopPropagation()">Visit our Facebook</a>
            </div>
            <div class="contact-item">📱 <strong>Mobile:</strong>
          ...
          <div class="payment-box">
            ...
            <div class="payment-options">
              <span>💵 Cash</span><span>📱 GCash</span><span>🏦 Bank Transfer</span>
            </div>
          </div>
        </div>
      </div>`;
...
function contactSeller() {
    window.open('https://www.facebook.com/armanstephent', '_blank', 'noopener');
}
